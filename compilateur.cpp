...
enum DATATYPE {UNSIGNED_INT, BOOLEAN, DOUBLE, CHAR, TYPE_UNKNOWN};
...
map<string, DATATYPE> DeclaredVariables;
...
DATATYPE TypeOf(const string &id){
...
// Factor := Number | Float | Char | Identifier | "(" Expression ")"| "!" Factor
// Number := Digit{Digit}
...
DATATYPE Identifier(void){
	...
	if(!IsDeclared(nom.c_str()))
	{
		cerr << "Erreur "<<nom<< "pas déclarée"<<endl;
		exit(-1);
	}

	if(TypeOf(nom) == CHAR) {
		cout << "\tmovzbq " << nom << ", %rax" << endl;
		cout << "\tpush %rax" << endl;
	} else {
		cout << "\tpush "<<nom<<endl;
	}
...
DATATYPE Char(void){
	cout <<"\tpush $"<<(int)lexer->YYText()[1]<<endl;
	current=(TOKEN) lexer->yylex();
	return CHAR;
}
	...
	while(current==MULOP){
		...
		if(type == DOUBLE)
		{
			...
			switch(mulop)
			{
				case MUL:
					...
					break;
				case DIV:
					cout << "\tfdivrp %st, %st(1)"<<endl;
					break;
				default:
					Error("Op interdit sur des DOUBLE seul * et / permis ");
			}
		...
		else
		{
			...
			switch(mulop)
			{
				case AND:
					if(nextType != BOOLEAN)
						Error("le type doit etre BOOLEAN pour l'operateur &&");
					cout << "\tmulq %rbx"<<endl;	// a * b -> %rdx:%rax
					cout << "\tpush %rax\t# AND"<<endl;	// store result
					break;
				case MUL:
					if(nextType != UNSIGNED_INT)
						Error("le type doit etre INTEGER pour l'operateur *");
					cout << "\tmulq %rbx"<<endl;	// a * b -> %rdx:%rax
...
DATATYPE Type(void){
	...
	if(strcmp(lexer->YYText(),"INTEGER")==0)
		...
		Error("Type inconnu (INTEGER, BOOLEAN, DOUBLE ou CHAR attendu)");
	current=(TOKEN) lexer->yylex();
	return type;
}
...
void VarDeclaration(void)
{
	...
	while(current==COMMA) 
	{
		current=(TOKEN) lexer->yylex(); // consomme la virgule
		if(current!=ID)
			Error("identificateur attendu");
	...
	for(unsigned int i = 0; i < idents.size(); i++){
		...
		if(IsDeclared(id.c_str())){
			cerr << "Erreur " << id << " deja declare" << endl;
			exit(-1);
		}
		...
		if(type == DOUBLE)
			...
			cout << id << ":\t.quad 0"<<endl;	// int et bool

		DeclaredVariables[id]=type;
	}
}
...
void VarDeclarationPart(void){
	...
	cout << "FormatStringD:\t.string \"%f\\n\""<<endl;
	cout << "FormatStringC:\t.string \"%c\\n\""<<endl;
...
DATATYPE Expression(void){
	...
	if(current==RELOP){
		...
		if(type != nextType)
			Error("Erreurtype incompatibles");
...
void AssignementStatement(void){
	...
	{
		cerr << "Erreur: " << lexer->YYText() << " pas declare" << endl;
		exit(-1);
	}
	variable=lexer->YYText();
	DATATYPE varType = TypeOf(variable);
	current=(TOKEN) lexer->yylex();
	if(current!=ASSIGN)
		Error(":= attendu");
...
void ForStatement(void)
{
	...
	if(!IsDeclared(variable.c_str())){ //verif si la variable est déclaree
		cerr << "Erreur : " << variable << " non declaree" << endl;
		exit(-1);
	}
	if(TypeOf(variable) != UNSIGNED_INT)
		Error("La variable de la boucle doi etre de type int");
	...
	if(Expression() != UNSIGNED_INT)
		Error("La valeur initial de la boucle doi etre de type int");
	cout << "\tpop " << variable << endl;
	...
	if(Expression() != UNSIGNED_INT)
		Error("La valeur final de la boucle doi etre de type int");
	cout << "\tpop %rcx" << endl;
	...
	cout << "\tpush %rcx" << endl;		// stocke dans la pile car  rcx est ecrasé par printf
	Statement();
	cout << "\tpop %rcx" << endl;
...
{
	current = (TOKEN) lexer->yylex();	// consomme display
	...
	{
		cout << "\tpop %rdx" << endl;
		cout << "\tmovq $FormatStringC, %rsi" << endl;
		cout << "\tmovl $1, %edi" << endl;
		cout << "\tmovl $0, %eax" << endl;
		cout << "\tcall __printf_chk@PLT" << endl;
	}
		...
		cout << "\tmovq $FormatStringD, %rdi" << endl;
		cout << "\tmovl $1, %eax" << endl;
		cout << "\tpush %rbx" << endl;
		cout << "\tmovq %rsp, %rbx" << endl;
		cout << "\tandq $-16, %rsp" << endl;
		cout << "\tcall printf@PLT" << endl;
		cout << "\tmovq %rbx, %rsp" << endl;
		cout << "\tpop %rbx" << endl;
	}
	else
		Error("Erreur de type ne peut pas etre affiche");
}

// CaseStatement := "CASE" Expression "OF" CaseListElement {";" CaseListElement} "END"
// CaseListElement := CaseLabelList ":" Statement | <vide>
// CaseLabelList := constant {"," constant}
void CaseStatement(void)
{
	int tag = ++TagNumber;
	int branchNum = 0;
	current = (TOKEN) lexer->yylex(); // consomme case

	DATATYPE type = Expression();
	if(type != UNSIGNED_INT && type != CHAR)
		Error("il faut un int ou un char pour le case");

	if(current != KEYWORD || strcmp(lexer->YYText(), "OF") != 0)
		Error("OF attendu");
	current = (TOKEN) lexer->yylex(); // consomme of

	while(current == NUMBER || current == CHARCONST)
	{
		branchNum++;
		int copie = branchNum;
		...
		while(current == NUMBER || current == CHARCONST)
		{
			...
			if(current == NUMBER)
				...
				val = (int)lexer->YYText()[1];

			current = (TOKEN) lexer->yylex(); // consomme la constante

			cout << "\tmovq (%rsp), %rax" << endl;
			cout << "\tcmpq $" << val << ", %rax" << endl;
			cout << "\tje Corps_" << tag << "_" << copie << endl; // si egal : va au corps

			if(current == COMMA) // il y a une autre constante dans la liste
				current = (TOKEN) lexer->yylex(); // consomme la virgule
		}

	
		cout << "\tjmp FinBranche_" << tag << "_" << copie << endl;

		if(current != COLON)
			Error(": attendu ");
		current = (TOKEN) lexer->yylex(); // consomme :

		cout << "Corps_" << tag << "_" << copie << ":" << endl;
		Statement();

		cout << "\tjmp FinCase_" << tag << endl; // sort du case
		cout << "FinBranche_" << tag << "_" << copie << ":" << endl;

		if(current == SEMICOLON) // consomme  ; 
			current = (TOKEN) lexer->yylex();
	}
	...
	if(current != KEYWORD || strcmp(lexer->YYText(), "END") != 0)
		Error("end a la fin du case");
	current = (TOKEN) lexer->yylex(); // consomme end

	cout << "FinCase_" << tag << ":" << endl;
	cout << "\taddq $8, %rsp" << endl; // eneleve la valeur sauvegardee sur la pile
}
