...
DATATYPE Type(void){
	...
	if(strcmp(lexer->YYText(),"INTEGER")==0)
		...
		Error("Type inconnu (INTEGER ou BOOLEAN attendu)");
	current=(TOKEN) lexer->yylex();
	return type;
}
...
void DisplayStatement(void){
	...
	if(type != UNSIGNED_INT)
		Error("Erreur de type, il faut un unsigned int");
...
void Program(void){
	if(current==KEYWORD && strcmp(lexer->YYText(),"VAR")==0)
		...
		// Pas de déclaration, on génère quand même FormatString1 pour DISPLAY
		cout << "\t.data"<<endl;
		cout << "\t.align 8"<<endl;
		cout << "FormatString1:\t.string \"%llu\\n\""<<endl;
	}
